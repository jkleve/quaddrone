    ...
    ground_( ground::Ground::reference() )
{
    ...
    //TWSR = 0;
    cbi(TWSR, TWPS0);
    cbi(TWSR, TWPS1); // TODO page 403 says cbi and sbi can only be used
    // TODO with registers 00 to 1F
...
void twi::TwiMgr::waitUntilIdle()
{
    //uint8_t tries = 0;
     while ((TWCR & (1 << TWINT)) == 0) {
         //if (++tries % 100 == 0)
         //   ground_.sendRegister(reg::TWI_CONTROL, TWCR);
     }
    ground_.sendRegister(reg::TWI_CONTROL, TWCR);
    ground_.sendRegister(reg::REG_TWI_STATUS, TWSR);
}
